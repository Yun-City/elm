...
let WX_PUSH_TOKEN = 'AT_dxSVOxQSlcgc2F7xQCq0MmxfrnYm2L5V';
...
async function sendNotify(
    ...
    desp,
    params = {},
    author = '\n\n关注小丑哥，羊毛不断。（本通知为人工呆瓜乐园币的通知自助开通/续费地址faka.taacc.cn）',
) {
    //提供6种通知
...
function wxPushNotify(text, desp, params) {
    ...
    return new Promise((resolve) => {
        if (WX_PUSH_TOKEN) {
            // 校验传入的WX_PUSH_TOKEN是否有效
            if (!WX_PUSH_TOKEN) {
                console.log('您所提供的WX_PUSH_TOKEN无效\n');
                resolve();
                return;
            }
            const options = {
